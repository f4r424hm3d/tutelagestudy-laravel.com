      ...
      <div class="row">
        <div class="col-12">
          <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-18">{{ $page_title }} <span class="text-danger">({{ $exam->exam_name }} -
                {{ $exampage->page_name }})</span></h4>
                ...
                <tbody>
                  ...
                  @foreach ($rows as $row)
                    <tr id="row{{ $row->id }}">
                      <td>{{ $i }}</td>
                      <td>
                        <input onchange="updatePriority('{{ $row->id }}', this.value)" style="width:60px!important;"
                          type="number" value="{{ $row->priority }}" min="1">
                      </td>
    ...
    function updatePriority(id, val) {
      ...
      $.ajax({
        ...
        success: function(data) {
          console.log(data); // See the response here for debugging
          if (data.success) {
            //location.reload(true);
            var h = 'Success';
            var msg = 'Priority has been updated.';
            var type = 'success';
            $('#toastMsg').text(msg);
            $('#liveToast').show();
